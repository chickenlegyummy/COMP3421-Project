/**
 * Accessories Page JavaScript - API Version
 * Handles filtering, sorting, pagination, and accessory display using backend API
 */
...
class AccessoriesPage {
    ...
    async loadProducts() {
        try {
            ...
            if (this.filters.category.length > 0) {
                params.append('category', this.filters.category[0]);
            }
            if (this.filters.brand.length > 0) {
                params.append('brand', this.filters.brand.join(','));
            }
            if (this.filters.price.length > 0) {
                const priceFilter = this.filters.price[0];
                if (priceFilter === '0-50') {
            ...
            // Fetch from API
            const response = await apiRequest(`${API.ACCESSORIES.LIST}?${params.toString()}`);
            
            this.allProducts = response.accessories;
            this.filteredProducts = response.accessories;
    ...
    async addToCart(productId) {
        try {
            await cartManager.addItem('accessory', productId, 1);
            
            // Show success message
            this.showNotification('Accessory added to cart!', 'success');
        } catch (error) {
            console.error('Error adding to cart:', error);
            this.showNotification('Failed to add accessory to cart', 'error');
        }
    }
...
// Global function to add accessory to cart from accessory cards
async function addToCartAccessory(accessoryId) {
    try {
        await cartManager.addItem('accessory', accessoryId, 1);
    } catch (error) {
